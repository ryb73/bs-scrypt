...
[@bs.module "scrypt"]
external paramsSync:
    float => (~maxmem : int=?) => (~maxmemfrac : float=?) => unit => params = "paramsSync";

[@bs.module "scrypt"]
external params:
    float => (~maxmem : int=?) => (~maxmemfrac : float=?) => unit
    => Js.Promise.t(params) = "paramsSync";

[@bs.module "scrypt"]
external hash : string => params => int => string => Js.Promise.t(Buffer.t) = "hash";
